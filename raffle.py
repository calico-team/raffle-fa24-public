...
def main():
    ...
    while len(dist) > 0 and total > 0 and any(inventory.values()):# while there are contestants and prizes remaining
        ...
        for i in range(1, len(inventory)+1):
            if i in person_prefs: # if failed, this means they didnt fill out all preferences
...
def get_preferences():
    ...
    for index, row in df.iterrows():
        ...
        identifier = str(curr_name) + ' (' + str(row[REGISTRATION_TEAM_NAME_COL_IDX]) + ')'
...
def load_identifier_to_team():
    ...
    for index, row in df.iterrows():
        ...
        identifier = str(row[TEAM_MEMBER_COL_IDX]) + ' (' + str(team) + ')'
...
def get_ticket_dist(identifier_to_team, team_to_tickets):
    ...
    for identifier in identifier_to_team:
        ...
        if team in team_to_tickets: # if failed, then this team is found in registrations but not on scoreboard?
